...
impl CustomProcessor<AnvilEthSigners> for ParseAnvilData {
	async fn process_receiver(
		&self,
		receiver: &mut Receiver<String>,
	) -> Result<Option<AnvilEthSigners>, FulfillError> {
...
impl Kestrel {
	...
	pub async fn run(&self) -> Result<(), anyhow::Error> {
		let eth_signers: State<AnvilEthSigners> = State::new();

		let anvil_eth_signers = eth_signers.clone();
		let anvil = kestrel::task(async move {
			// construct the keys fulfiller
			let keys_fulfiller = Custom::new(anvil_eth_signers.write(), ParseAnvilData);
		...
		let signers = eth_signers.read().wait_for().await;
