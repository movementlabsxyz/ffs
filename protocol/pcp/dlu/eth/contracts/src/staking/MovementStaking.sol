    ...
    ReentrancyGuard
{
    ...
    /// @notice Sets the accepting epoch for a given domain
    ...
    function setAcceptingEpoch(address domain, uint256 newEpoch) external {
        require(newEpoch <= getEpochByL1BlockTime(address(domain)), "NEW_EPOCH_MUST_BE_LESS_THAN_PRESENT_EPOCH");
        require(newEpoch > getAcceptingEpoch(domain), "NEW_EPOCH_MUST_BE_HIGHER_THAN_CURRENT_EPOCH");
        require(msg.sender == domain, "UNAUTHORIZED");
        currentAcceptingEpochByDomain[domain] = newEpoch;
    }
