...
/**
 * Development deployment script for the Multi-Commit-Rollup (MCR) system.
 * This deploys a test environment with short epochs and quick postconfirmer rotations.
 * Includes MOVE token for staking, Movement Staking for managing attesters, and MCR for cross-chain settlement.
 */
...
contract DeployMCRDev is Script {
    function run() external {
        ...
        // Set up MCR with MOVE token as the only custodian for rewards
        address[] memory custodians = new address[](1);
        custodians[0] = address(moveTokenProxy);

        // Deploy MCR behind proxy with test configuration
        ...
        console.log("MCR Configuration:");
        MovementStaking staking = MovementStaking(movementStakingProxy);
        uint256 epochDuration = staking.getEpochDuration(mcrProxy);
        uint256 postconfirmerDuration = mcr.getPostconfirmerDuration();
        ...
        uint256 stakingEpochDuration = MovementStaking(movementStakingProxy).epochDurationByDomain(mcrProxy);
        console.log("- Epoch Duration for MCR domain: %s seconds", stakingEpochDuration);
        require(stakingEpochDuration == 10, "Incorrect staking epoch duration");

        // Some simple sanity checks
        console.log("\ngetAcceptingEpoch(mcrProxy): %s", staking.getAcceptingEpoch(mcrProxy));
        console.log("\ngetLastPostconfirmedSuperBlockHeight(): %s", mcr.getLastPostconfirmedSuperBlockHeight());
        console.log("\nList of active attesters:");
        address[] memory stakedAttesters = staking.getStakedAttestersForAcceptingEpoch(mcrProxy);
        ...
        console.log("\nPostconfirmer: %s", mcr.getPostconfirmer());
